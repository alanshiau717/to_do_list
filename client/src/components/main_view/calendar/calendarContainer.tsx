...
function CalendarContainer(props: Props) {
  ...
  function getTasksFromCurrentList(
    listId: String,
    folderId: String,
  ): GetFoldersQuery["folders"][0]["lists"][0]["tasks"] {
  ...
  const [addEvent, createTaskScheduleResponse] = useMutation(
    CreateTaskScheduleDocument,
  );
  useEffect(() => {
    if (error) {
  ...
  useEffect(() => {
    if (createTaskScheduleResponse.error) {
      console.log(
        JSON.stringify(createTaskScheduleResponse.error, null, 2),
      );
  ...
  function eventRecieveCallBack(args: EventReceiveArg) {
    ...
    if (event.start != null && event.end != null) {
      addEvent({
        variables: {
          data: {
            taskId: parseInt(event.extendedProps.taskId),
