...
//TO-DO Figure out a way to do this easier
//TO-DO Figure out better way to declare Data and Error types
...
exports.findAll = (req: Request, res: Response) => {
  ...
  if (typeof title == "string") {
    ...
    // if we are not given a title, just return everything
    Tutorial.find()
      .then((data: any) => {
        res.send(data);
      })
...
exports.delete = (req: Request, res: Response) => {
  ...
  Tutorial.findByIdAndRemove(id).then((data: any) => {
    if (!data) {
      res.status(404).send({
